...
public class RabbitMQConfig {
    ...
    public AmqpTemplate amqpTemplate() {
        ...
        // 发送失败发送回来 即 returncallback 函数
        rabbitTemplate.setReturnCallback((message, replyCode, replyText, exchange, routingKey) -> {
            String correlationId = message.getMessageProperties().getCorrelationId();
            String body = new String(message.getBody());
            log.debug("消息内容：{} 消息id：{} 发送失败, 应答码：{} 原因：{} 交换机: {}  路由键: {}", body, correlationId, replyCode, replyText, exchange, routingKey);
        });
...
//       x-dead-letter-routing-key    声明 死信路由键
        args.put(DEAD_LETTER_ROUTING_KEY, "KEY_R222");
    ...
    public Binding deadLetterBinding() {
        return new Binding("DL_QUEUE", Binding.DestinationType.QUEUE
                , "DL_EXCHANGE"
                , "DL_KEY"
                , null);
    }
    ...
    public Binding redirectBinding() {
        return new Binding("REDIRECT_QUEUE"
